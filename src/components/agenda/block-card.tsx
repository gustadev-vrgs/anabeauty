...
export function BlockCard({ block, className }: BlockCardProps) {
  return (
    <div
      className={cn(
        'w-full rounded-2xl border border-coffee-cappuccino bg-coffee-latte/45 px-3 py-2 text-coffee-darkRoast shadow-sm sm:px-4 sm:py-3',
        className,
      )}
    >
      ...
      <p className="text-xs text-coffee-espresso sm:text-sm">{block.reason ?? 'Horário indisponível'}</p>
    </div>
  );
}
